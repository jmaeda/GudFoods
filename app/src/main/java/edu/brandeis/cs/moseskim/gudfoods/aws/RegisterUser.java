...
//                    password.setBackground(getDrawable(R.drawable.text_border_error));
                ...
                if (userInput != null) {
                    if (userInput.length() > 0) {
                        userAttributes.addAttribute(AWSService.getSignUpFieldsC2O().get(email.getHint()).toString(), userInput);
                    }
                }
                ...
                if (userInput != null) {
                    if (userInput.length() > 0) {
                        userAttributes.addAttribute(AWSService.getSignUpFieldsC2O().get(phone.getHint()).toString(), userInput);
                    }
                }

                showWaitDialog("Signing up...");

                AWSService.getPool().signUpInBackground(usernameInput, userpasswordInput, userAttributes, null, signUpHandler);
...
//            username.setBackground(getDrawable(R.drawable.text_border_error));
            showDialogMessage("Sign up failed",AWSService.formatException(exception),false);
        }
    };
